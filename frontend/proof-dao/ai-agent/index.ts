...
// Types for ProofDAO entities (matching your existing types)
interface Task {
  id: string;
  title: string;
  description: string;
...
class AIAgent {
   ...
   */
  private combineResults(primaryReview: any, qualityMetrics: QualityMetrics, fraudCheck: any, feedback: any): TaskValidationResult {
    // Calculate weighted final score
    const finalScore = Math.round(
      (primaryReview.score * 0.4) +
      (qualityMetrics.overallQuality * 0.3) +
      ((100 - fraudCheck.riskScore) * 0.3)
    );

    // Determine approval recommendation
    const shouldApprove = finalScore >= 75 && 
                         fraudCheck.riskScore < 30 && 
                         primaryReview.meetsRequirements;
    ...
    const flaggedIssues = [
      ...primaryReview.keyIssues || [],
      ...fraudCheck.suspiciousFlags || []
    ];
...
// Usage example and integration with your DAO dashboard
export class TaskValidationService {
  ...
  constructor() {
    ...
    this.aiAgent = new AIAgent(process.env.NEXT_PUBLIC_GROQ_API_KEY);
  }

  /**
   * Validate a submission and return results compatible with your UI
   */
  async validateSubmission(task: Task, submission: Submission): Promise<{
    aiScore: number;
  ...
  async processPendingSubmissions(daoId: string, submissions: Submission[], tasks: Task[]) {
    ...
    results.forEach((result, index) => {
      ...
      console.log(`Submission ${submission.id}: ${result.score}/100 - ${result.shouldApprove ? '✅ APPROVE' : '❌ REVIEW'}`);
    });
...
// Integration example for your existing DAO dashboard
export function integrateAIValidation() {
  const validationService = new TaskValidationService();

  // Add this to your submission review workflow
  const enhanceSubmissionReview = async (submission: Submission, task: Task) => {
    try {
      const aiValidation = await validationService.validateSubmission(task, submission);
      
      return {
        ...submission,
        aiScore: aiValidation.aiScore,
        aiRecommendation: aiValidation.aiRecommendation,
        aiFeedback: aiValidation.aiFeedback,
        aiFlags: aiValidation.aiFlags,
        aiConfidence: aiValidation.confidence
      };
    } catch (error) {
      console.error('AI validation failed:', error);
      return submission; // Fallback to manual review
    }
  };

  return { enhanceSubmissionReview, validationService };
}

// Environment setup validation
if (!process.env.NEXT_PUBLIC_GROQ_API_KEY) {
  console.error(`
❌ Missing NEXT_PUBLIC_GROQ_API_KEY environment variable!

To set it up:
1. Get your API key from: https://console.groq.com/keys
2. Add to your environment:
   - Unix/MacOS: export NEXT_PUBLIC_GROQ_API_KEY=your-api-key
   - Windows: $env:NEXT_PUBLIC_GROQ_API_KEY = "your-api-key"
   - .env file: NEXT_PUBLIC_GROQ_API_KEY=your-api-key
  `);
}
